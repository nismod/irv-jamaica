...
def create_single_band_raster(
    ...
    data = (
        ...
        else np.random.random((height, width))
    )
...
if __name__ == "__main__":
    ...
    create_single_band_raster(
        "single_band/raster_ref.tif",
        16,
        16,
        value=42,
        nodata_value=-9999,
        nodata_frequency=0.8,
    )
    create_single_band_raster(
        "single_band/raster_16x16.tif",
        16,
        16,
        nodata_value=np.nan,
        nodata_frequency=0.2,
    )
    create_single_band_raster(
        "single_band/raster_8x8.tif", 8, 8, nodata_value=-9999, nodata_frequency=0.3
    )
    create_single_band_raster(
        "single_band/raster_32x32.tif", 32, 32, nodata_value=0, nodata_frequency=0.1
    )

    # Generate a multi-band raster with nodata values
    create_multi_band_raster(
        "multiband_raster.tif",
        16,
        16,
        band_count=4,
        nodata_value=-9999,
