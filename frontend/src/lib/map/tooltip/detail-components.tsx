...
export const CoastalDefenceDetails: FC<DetailsComponentProps> = ({ f }) => {
  ...
  const filteredProtectedFeatures = data.filter((feature) => feature.id !== f.uid);
  ...
  return (
    <>
      <List>
        ...
        <DataItem
          label="Max modelled flood height (m)"
          value={`${numFormat(f.max_flood_height)}`}
        />
              ...
              sortedProtectedFeatures.length > 0 ? (
                <TableContainer component={Paper} sx={{ mt: 1, maxHeight: 300 }}>
                  <Table
                    size="small"
                    stickyHeader
                    sx={{ '& .MuiTableCell-root': { padding: '4px 8px', fontSize: '0.75rem' } }}
                  >
                    <TableHead>
                      <TableRow>
                      ...
                      {sortedProtectedFeatures.map((feature, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell align="right">
                            {numFormat(feature.avoided_eael_mean)}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
